...
- **OKR draft** (prominent, top of sidebar) — the user's real H2 direction, rebuilt **natively** in the hub design language (English). Data was extracted from their `h2-direction-tracker.html` into `OKR_AREAS` (4 objectives + a "Questions to Sit With" reflection area) and `OKR_KRS` (16 key results + 5 reflection prompts), each with priority (P0–P2), confidence (Confirmed/Likely/Hypothesis), metric/target, and a detail line. Rendered as objective cards (accent bar, confidence pill, weight, live progress rollup) with key-result rows whose status dots reuse the hub's `state.s` + `cycle()` (open → in progress → done; seeded once via `seedOKR()` from the tracker's active/next/backlog status). Reflection prompts render as a numbered list. Standard `.mainwrap` width. NOTE: the earlier iframe approach + `deploy/okr.html` were **removed** — the user wanted it native, not embedded. Replaced Weeks/Objectives as the primary planning surface (old plan kept for reference).
